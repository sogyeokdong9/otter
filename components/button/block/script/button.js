...
(function () {
  ...
  for (let i = 0; i < trgElmn.length; i++) {
    ...
    const properties = element.dataset.buttonProperties;
    switch (properties) {
      case 'block':
        setClassToButtonType( element, 'otter-btn-block' );
        break;
    }
    ...
    function logButton(info, idx) {
      ...
      const loadButtonItmeProperties = returnCheckValue( trgElmn[idx].getAttribute('data-button-properties'), '(properties)' );
      const groupOfPairs = [
        ...
        { id: 7, name: loadButtonItmeProperties, class: 'trigger-properties' }
      ]
